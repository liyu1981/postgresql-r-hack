...
static const struct config_enum_entry replication_level_options[] = {
	{"sync", XACT_REPL_SYNC, false},
	{"eager", XACT_REPL_EAGER, false},
	{"lazy", XACT_REPL_LAZY, false},
	{NULL, 0}
};
...
static char *XactReplLevel_string;
...
static struct config_bool ConfigureNamesBool[] =
{
...
#ifdef REPLICATION
	{
		{"replication", PGC_SIGHUP, REPLICATION_OPTIONS,
			gettext_noop("Enables replication via a Group Communication System."),
			NULL
		},
		&replication_enabled,
		false, NULL, NULL
	...
	{
		{"replication_seed", PGC_SIGHUP, REPLICATION_OPTIONS,
			gettext_noop("Allow this node to seed the distributed database system."),
			NULL
		},
		&replication_may_seed,
		false, NULL, NULL
	},
#endif
...
static struct config_int ConfigureNamesInt[] =
{
	...
	},
	{
		...
		50, 1, MAX_BACKENDS, assign_max_background_workers, NULL
	},
...
static struct config_string ConfigureNamesString[] =
{
	...
	{
		{"transaction_replication", PGC_USERSET, CLIENT_CONN_STATEMENT,
			gettext_noop("Sets the current transaction's replication level."),
			NULL,
			GUC_NO_RESET_ALL | GUC_NOT_IN_SAMPLE | GUC_DISALLOW_IN_FILE
		},
		&XactReplLevel_string,
		NULL, assign_XactReplLevel, show_XactReplLevel
	},
...
#ifdef REPLICATION
	{
		{"replication_gcs", PGC_SIGHUP, REPLICATION_OPTIONS,
			gettext_noop("Sets the group communication system to use for "
						 "replication."),
			NULL,
			GUC_IS_NAME
		},
	...
	{
		{"replication_group", PGC_SIGHUP, REPLICATION_OPTIONS,
			gettext_noop("Sets the name of the GCS group to use for "
						 "replication."),
			NULL,
			GUC_IS_NAME
		},
...
static struct config_enum ConfigureNamesEnum[] =
{
	...
	{
		{"default_transaction_replication", PGC_USERSET, CLIENT_CONN_STATEMENT,
			gettext_noop("Sets the transaction replication level of each "
						 "new transaction."),
			gettext_noop("Each SQL transaction has a replication level, which "
						 "can be either \"sync\", \"eager\", or \"lazy\".")
		},
		&DefaultXactReplLevel,
		XACT_REPL_SYNC, replication_level_options, NULL, NULL
	},
		...
		case VAR_SET_MULTI:
			...
			if (strcmp(stmt->name, "TRANSACTION") == 0)
			{
				...
				foreach(head, stmt->args)
				{
					...
					else if (strcmp(item->defname, "transaction_replication") == 0)
						SetPGVariable("transaction_replication",
									  list_make1(item->arg), stmt->is_local);
			...
			else if (strcmp(stmt->name, "SESSION CHARACTERISTICS") == 0)
			{
				...
				foreach(head, stmt->args)
				{
					...
					if (strcmp(item->defname, "transaction_isolation") == 0)
						...
						SetPGVariable("default_transaction_replication",
									  list_make1(item->arg), stmt->is_local);
					else if (strcmp(item->defname, "transaction_read_only") == 0)
...
{
	if (newval + max_background_workers + 1 > MAX_BACKENDS)
		return false;
