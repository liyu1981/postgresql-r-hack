...
%type <str>		iso_level repl_level opt_encoding
%type <node>	grantee
...
%token <keyword> ABORT_P ABSOLUTE_P ACCESS ACTION ADD_P ADMIN AFTER
	...
	EACH EAGER ELSE ENABLE_P ENCODING ENCRYPTED END_P ENUM_P ESCAPE EXCEPT
	...
	LANGUAGE LARGE_P LAST_P LAZY LC_COLLATE_P LC_CTYPE_P LEADING
	...
	RESET RESTART RESTRICT RETURNING RETURNS REVOKE RIGHT ROLE
...
VariableResetStmt:
			...
			| RESET TRANSACTION REPLICATION_P LEVEL
				{
					VariableSetStmt *n = makeNode(VariableSetStmt);
					n->kind = VAR_RESET;
					n->name = "transaction_replication";
					$$ = (Node *) n;
				}
...
VariableShowStmt:
			...
			| SHOW TRANSACTION REPLICATION_P LEVEL
				{
					VariableShowStmt *n = makeNode(VariableShowStmt);
					n->name = "transaction_replication";
					$$ = (Node *) n;
...
transaction_mode_item:
			ISOLATION LEVEL iso_level
					{ $$ = makeDefElem("transaction_isolation",
									   makeStringConst($3, @3)); }
			| REPLICATION_P LEVEL repl_level
					{ $$ = makeDefElem("transaction_replication",
									   makeStringConst($3, @3)); }
			| READ ONLY
