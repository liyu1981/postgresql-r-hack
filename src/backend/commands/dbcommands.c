...
#ifdef REPLICATION
#include "commands/replicacmds.h"
...
createdb(const CreatedbStmt *stmt)
{
	...
	new_record[Anum_pg_database_datreplgroup - 1] = 
		DirectFunctionCall1(namein, CStringGetDatum(dbname));
...
dropdb(const char *dbname, bool missing_ok)
{
	...
	if (replication_enabled)
		ereport(ERROR,
				(errcode(ERRCODE_OBJECT_IN_USE),
				 errmsg("database \"%s\" is being replicated",
						dbname),
				 errhint("Unable to drop replicated databases.")));
#endif
	 ...
	 *
	 * As CheckOtherDBBackends() waits for up to five seconds, we can safely
	 * ignore all helper backends, which will terminate within that time,
	 * provided they aren't busy.
	 *
...
RenameDatabase(const char *oldname, const char *newname)
{
...
#ifdef REPLICATION
	if (replication_enabled)
		elog(ERROR, "Unable to rename database in replicated mode.");
#endif
	...
	/*
	 * We don't support renaming of actively replicated databases, yet.
	 */
#ifdef REPLICATION
	if (replication_enabled)
		ereport(ERROR,
				(errcode(ERRCODE_OBJECT_IN_USE),
				 errmsg("cannot rename database \"%s\" because it is replicated",
						oldname)));
#endif
...
AlterDatabase(AlterDatabaseStmt *stmt, bool isTopLevel)
{
	 ...
	 * connections.
	 *
	 * Except if we switch on replication, see below.
	 */
	rel = heap_open(DatabaseRelationId, RowExclusiveLock);
	ScanKeyInit(&scankey,
				Anum_pg_database_datname,
...
#ifdef REPLICATION
	if (replication_enabled)
		elog(ERROR, "Unable to alter database in replicated mode.");
#endif
