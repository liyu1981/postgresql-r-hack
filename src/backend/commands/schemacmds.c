...
CreateSchemaCommand(CreateSchemaStmt *stmt, const char *queryString)
{
	...
	if (replication_enabled && !IsBackgroundWorkerProcess())
		elog(ERROR, "Unable to create schema in replicated mode.");
#endif
...
#ifdef REPLICATION
	if (replication_enabled && !IsBackgroundWorkerProcess())
		elog(ERROR, "Unable to drop schema in replicated mode.");
#endif
...
RenameSchema(const char *oldname, const char *newname)
{
	...
	if (replication_enabled)
		elog(ERROR, "Unable to rename schema in replicated mode.");
#endif
...
#ifdef REPLICATION
	if (replication_enabled)
		elog(ERROR, "Unable to alter schema in replicated mode.");
#endif
