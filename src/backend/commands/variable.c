...
/*
 * SET TRANSACTION REPLICATION LEVEL
 */

const char *
assign_XactReplLevel(const char *value, bool doit, GucSource source)
{
	if (FirstSnapshotSet)
	{
		ereport(GUC_complaint_elevel(source),
				(errcode(ERRCODE_ACTIVE_SQL_TRANSACTION),
				 errmsg("SET TRANSACTION REPLICATION LEVEL must be called before any query")));
		/* source == PGC_S_OVERRIDE means do it anyway, eg at xact abort */
		if (source != PGC_S_OVERRIDE)
			return NULL;
	}
	else if (IsSubTransaction())
	{
		ereport(GUC_complaint_elevel(source),
				(errcode(ERRCODE_ACTIVE_SQL_TRANSACTION),
				 errmsg("SET TRANSACTION REPLICATION LEVEL must not be called in a subtransaction")));
		/* source == PGC_S_OVERRIDE means do it anyway, eg at xact abort */
		if (source != PGC_S_OVERRIDE)
			return NULL;
	}
	...
	{
		if (doit)
			XactReplLevel = XACT_REPL_SYNC;
	}
	else if (strcmp(value, "eager") == 0)
	{
		if (doit)
			XactReplLevel = XACT_REPL_EAGER;
	}
	else if (strcmp(value, "eager") == 0)
	{
		if (doit)
			XactReplLevel = XACT_REPL_LAZY;
	}
	else if (strcmp(value, "default") == 0)
	{
		if (doit)
...
const char *
show_XactReplLevel(void)
{
	switch (XactReplLevel)
	{
		case XACT_REPL_SYNC:
			return "sync";
		case XACT_REPL_EAGER:
			return "eager";
		case XACT_REPL_LAZY:
			return "lazy";
		default:
