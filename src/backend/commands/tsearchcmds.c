...
DefineTSParser(List *names, List *parameters)
{
	...
	if (replication_enabled)
		elog(ERROR, "Unable to create text seacrh parser in replicated mode.");
#endif
...
#ifdef REPLICATION
	if (replication_enabled)
		elog(ERROR, "Unable to drop text seacrh parser "
			 "in replicated mode.");
#endif
...
#ifdef REPLICATION
	if (replication_enabled)
		elog(ERROR, "Unable to rename text seacrh parser "
			 "in replicated mode.");
#endif
...
#ifdef REPLICATION
	if (replication_enabled)
		elog(ERROR, "Unable to create text search dictionary "
			 "in replicated mode.");
#endif
...
#ifdef REPLICATION
	if (replication_enabled)
		elog(ERROR, "Unable to rename text search dictionary "
			 "in replicated mode.");
#endif
...
#ifdef REPLICATION
	if (replication_enabled)
		elog(ERROR, "Unable to drop text search dictionary "
			 "in replicated mode.");
#endif
...
#ifdef REPLICATION
	if (replication_enabled)
		elog(ERROR, "Unable to alter text search dictionary "
			 "in replicated mode.");
#endif
...
#ifdef REPLICATION
	if (replication_enabled)
		elog(ERROR, "Unable to create text search template "
			 "in replicated mode.");
#endif
...
#ifdef REPLICATION
	if (replication_enabled)
		elog(ERROR, "Unable to rename text search template "
			 "in replicated mode.");
#endif
...
#ifdef REPLICATION
	if (replication_enabled)
		elog(ERROR, "Unable to remove text search template "
			 "in replicated mode.");
#endif
...
#ifdef REPLICATION
	if (replication_enabled)
		elog(ERROR, "Unable to create text search configuration "
			 "in replicated mode.");
#endif
...
#ifdef REPLICATION
	if (replication_enabled)
		elog(ERROR, "Unable to rename text search configuration "
			 "in replicated mode.");
#endif
...
#ifdef REPLICATION
	if (replication_enabled)
		elog(ERROR, "Unable to drop text search configuration "
			 "in replicated mode.");
#endif
...
#ifdef REPLICATION
	if (replication_enabled)
		elog(ERROR, "Unable to alter text search configuration "
			 "in replicated mode.");
#endif
...
#ifdef REPLICATION
	if (replication_enabled)
		elog(ERROR, "Unable to make text search configuration mapping "
			 "in replicated mode.");
#endif
...
#ifdef REPLICATION
	if (replication_enabled)
		elog(ERROR, "Unable to drop text search configuration mapping "
			 "in replicated mode.");
#endif
