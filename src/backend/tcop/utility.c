...
#ifdef REPLICATION
			/*
			 * The recovery subscriber must be able to create a databese
			 * from within it's adaption function, so we allow that.
			 *
			 * As long as the recovery subscriber doesn't fail, that seems
			 * save. However, we better cope with errors somehow...
			 */
			if (!IsBackgroundWorkerProcess() ||
				!IsRecoverySubscriber(MyWorkerInfo))
#endif
