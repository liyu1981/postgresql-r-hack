...
ProcessInterrupts(void)
{
...
#ifdef REPLICATION
	/*
	 * If the abortFlag has been rised, we either switch to background
	 * worker cleanup mode or emit a warning to the connected client.
	 */
	if (MyProc->abortFlag)
	{
		ImmediateInterruptOK = false;	/* not idle anymore */
		if (IsBackgroundWorkerProcess())
		{
			...
			BgWorkerCleanupInProgress = true;
		}
		else
		{
			QueryCancelPending = true;
			if (DoingCommandRead)
				ereport(WARNING,
						(errcode(ERRCODE_T_R_SERIALIZATION_FAILURE),
						 errmsg("Transaction will be aborted due to conflicting remote transaction")));
			else
			{
				BgWorkerCleanupInProgress = true;
				ereport(ERROR,
						(errcode(ERRCODE_T_R_SERIALIZATION_FAILURE),
						 errmsg("could not serialize transaction due to concurrent remote transaction")));
			}
		}
	}
#endif
	...
	if (QueryCancelPending)
	{
		...
		{
			/*
			 * FIXME: Make sure user termination works fine for all kinds
			 *        of background jobs.
			 *
			 *        Check which job types allow interrupts during their
			 *        execution at all.
			 */
			Assert(MyWorkerInfo->wi_state == WS_IDLE ||
				   MyWorkerInfo->wi_state == WS_AUTOVACUUM);
		...
		if (!DoingCommandRead)
		{
			...
			ereport(ERROR,
					(errcode(ERRCODE_QUERY_CANCELED),
					 errmsg("canceling statement due to user request ([%d/%d])", MyProcPid, MyBackendId)));
		}
	}
...
#ifdef REPLICATION
	if (IsUnderPostmaster && replication_enabled)
	{
		rdb_state db_state = get_db_replication_state(MyDatabaseId);
		switch (db_state)
		{
			case RDBS_UNKNOWN:
				/* no user should ever see this, but you never know... */
				ereport(FATAL,
						(errcode(ERRCODE_CANNOT_CONNECT_NOW),
						 errmsg("database %s is not known to the replication "
								"system, yet.", dbname)));

			case RDBS_UNABLE_TO_CONNECT:
				ereport(FATAL,
						(errcode(ERRCODE_OPERATOR_INTERVENTION),
						 errmsg("Failed to connect to the communication "
								"system for replication.")));

			case RDBS_JOIN_REQUESTED:
				ereport(FATAL,
						(errcode(ERRCODE_CANNOT_CONNECT_NOW),
						 errmsg("Cannot connect to database %s, waiting for "
								"the communication system.", dbname)));

			case RDBS_AWAITING_MAJORITY:
				ereport(FATAL,
						(errcode(ERRCODE_CANNOT_CONNECT_NOW),
						 errmsg("Cannot connect to database %s, it is waiting "
								"for other nodes in the replication group.",
								dbname)));
			...
			case RDBS_RECOVERING_DATA:
				ereport(FATAL,
						(errcode(ERRCODE_CANNOT_CONNECT_NOW),
						 errmsg("Cannot connect to database %s, it is still "
								"recovering.", dbname)));
...
#ifdef REPLICATION
void
bgworker_db_state_change(IMessage *msg)
{
	buffer			b;
	rdb_state       new_state;
	...
	new_state = get_int8(&b);

	IMessageRemove(msg);

#ifdef DEBUG_CSET_APPL
	elog(DEBUG3, "bgworker: database %d changed to state %s",
		 MyDatabaseId, decode_database_state(new_state));
#endif
}

bool
bgworker_apply_cset(IMessage *msg)
{
	EState			   *estate;
	...
	/*
	 * Start a transaction for application of the remote change
	 * set. We need to do that before calling cset_init, so as to have
	 ...
	 * change sets, if required.
	 */
	cset_init();
	cset_prepare_reader(msg);

	/*
	 * Create an executor state for applying the remote transaction
...
bool
bgworker_commit_request(IMessage *msg)
{
	buffer			b;
	int             count_csets;
...
//#endif
	...
	if (csets_recvd_counter == count_csets)
	{
		if (!MyProc->abortFlag)
...
//#endif
			 ...
			 * get a TXN_ABORTED message from the origin node anyway. However,
			 * to make sure we don't get any more change sets from the
	...
	{
		elog(ERROR, "bg worker [%d/%d]: invalid change set count: %d vs %d",
			 MyProcPid, MyBackendId, csets_recvd_counter, count_csets);
	}
...
#ifdef DEBUG_CSET_APPL
	elog(DEBUG3, "bg worker [%d/%d]: received abort message",
		 MyProcPid, MyBackendId);
#endif
...
void
bgworker_seq_setval(IMessage *msg)
{
	buffer		b;
	char	   *seqname;
	RangeVar   *sequence;
	...
	/*
	 * FIXME: somehow, we have to make sure to apply multiple
	 *        SEQ_INCREMENTs and SEQ_SETVALs in the correct order.
	 */
	...
	set_ps_display("remote sequence setval", false);
...
#ifdef DEBUG_CSET_APPL
	elog(DEBUG3, "bg worker [%d/%d]: db %d: processing seq setval for '%s' (next: %lld, iscalled: %d)",
		 MyProcPid, MyBackendId, MyDatabaseId,
		 seqname, (long long int) next, (int) iscalled);
#endif

	relid = RangeVarGetRelid(sequence, false);
	result = DirectFunctionCall3(setval_oid, ObjectIdGetDatum(relid),
								 Int64GetDatum(next),
...
void
bgworker_recovery_restart(IMessage *msg)
{
	rdb_state db_state = get_db_replication_state(MyDatabaseId);

#ifdef DEBUG_CSET_APPL
	elog(DEBUG3, "bg worker [%d/%d]: working on database %d in state %s",
		 MyProcPid, MyBackendId, MyDatabaseId,
		 decode_database_state(db_state));
#endif

	/* FIXME: indentation */
				set_ps_display("schema recovery", false);

				Assert(IsRecoverySubscriber(MyWorkerInfo));
				Assert(db_state == RDBS_RECOVERING_SCHEMA);

                IMessageRemove(msg);

				ProcessSchemaRecoveryRestart();
}

void
bgworker_schema_adaption(IMessage *msg)
{
	rdb_state db_state = get_db_replication_state(MyDatabaseId);
				...
				ProcessSchemaAdaption(msg);
}

void
bgworker_recovery_data(IMessage *msg)
{
	rdb_state db_state = get_db_replication_state(MyDatabaseId);

				set_ps_display("recovery subscriber", false);

				Assert(IsRecoverySubscriber(MyWorkerInfo));
				Assert(db_state == RDBS_RECOVERING_SCHEMA ||
					   db_state == RDBS_RECOVERING_DATA);
				ProcessRecoveryData(msg);
}

void
bgworker_recovery_request(IMessage *msg)
{
	rdb_state db_state = get_db_replication_state(MyDatabaseId);

				set_ps_display("recovery provider", false);

				Assert(IsRecoveryProvider(MyWorkerInfo));
				Assert(db_state == RDBS_OPERATING);

				ProcessRecoveryRequest(msg);
}
