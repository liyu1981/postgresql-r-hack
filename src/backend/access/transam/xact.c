...
int			DefaultXactReplLevel = XACT_REPL_SYNC;
int			XactReplLevel;
...
StartTransaction(void)
{
	...
	XactReplLevel = DefaultXactReplLevel;
	XactReadOnly = DefaultXactReadOnly;
...
#ifdef REPLICATION
	if (replication_enabled && !IsCoordinatorProcess())
	{
		if (!IsBackgroundWorkerProcess() && CurrentCset)
		{
			/*
			 * Write an end-of-stream marker and close the write
			 * stream so a last change set gets flushed out, if
			 * any. Then wait for the commit ordering decision.
			 */
			if (CurrentCset->writer)
				istream_write_int8(CurrentCset->writer, CSCMD_EOS);

			istream_close_writer(CurrentCset->writer);

			if (csets_sent_counter > 0)
...
AbortTransaction(void)
{
	...
	{
		/*
		 * In case of synchronous replication as well as for eagerly
		 * replicated transactions which have already sent at least one
		 * change set, we need to tell all other nodes, that this transaction
		 * got aborted.
		 *
		 * Background workers have their own exception handling routine and
		 * don't need to inform the coordinator from here.
		 */
		if (!IsBackgroundWorkerProcess() &&
			(CurrentCset && csets_sent_counter > 0))
		{
			TransactionId xid = GetCurrentTransactionIdIfAny();
			Assert(TransactionIdIsValid(xid));

			send_txn_aborted_msg(xid, ERRCODE_TRANSACTION_ROLLBACK);
		}

		/*
		 * Reset transaction state used for replication
		 */
		MyProc->abortFlag = false;
		cset_reset();
	}
	...
	switch (s->blockState)
	{
		...
		case TBLOCK_SUBINPROGRESS:
			...
			if (replication_enabled && !IsBackgroundWorkerProcess() &&
				!IsCoordinatorProcess() && XactReplLevel == XACT_REPL_SYNC &&
				CurrentCset)
			{
				/*
				 * FIXME: fully synchronous transaction aren't implemented,
				 * yet. Should request a global_command_commit (not full
				 * transaction, but just the command).
				 */
#if 0
				if (CurrentCset)
					istream_close_writer(CurrentCset->writer);
