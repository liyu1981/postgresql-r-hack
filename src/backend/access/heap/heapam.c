...
l1:
	...
	heap_delete_internal(relation, &tp, buffer, cid);
...
void
heap_delete_internal(Relation relation, HeapTuple tuple, Buffer buffer,
					 CommandId cid)
{
	TransactionId xid = GetCurrentTransactionId();
	Page		page;
	...
	else if (HeapTupleHasExternal(tuple))
		toast_delete(relation, tuple);
...
l2:
	...
	heap_update_internal(relation, &oldtup, newtup, buffer, cid);
...
void
heap_update_internal(Relation relation, HeapTuple oldtup, HeapTuple newtup,
					 Buffer buffer, CommandId cid)
{
	TransactionId xid = GetCurrentTransactionId();
	HeapTuple	heaptup;
	Buffer		newbuf;
