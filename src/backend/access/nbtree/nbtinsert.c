		...
		if (offset <= maxoff)
		{
			...
			if (!ItemIdIsDead(curitemid))
			{
										 ...
										 &all_dead))
				{
					 ...
					 * being updated in another transaction. Just return the
					 * fact that it is a potential conflict and leave the
					 * full check till later.
					 */
...
#ifdef REPLICATION
					if (IsReplicatedBackend() || IsCsetApplicator(MyWorkerInfo))
					{
						CommitOrderId my_coid = InvalidCommitOrderId;

						/*
						 * If xwait is a valid transaction id, there's a
						 * concurrent transaction which we are in conflict
						 * with. In that case, we must respect the decided
						 * commit ordering.
						 *
						 * Otherwise, the conflicting transaction has
						 * already committed, so we must abort.
						 *
						 * FIXME: figure out what to do in the
						 * UNIQUE_CHECK_EXISTING case
						 *
						 * FIXME: this changes behavior (error number) of
						 * normal (replicated) backends.
						 */
						if (TransactionIdIsValid(xwait))
							resolve_conflict(xwait, &my_coid);
						else
						...
						{
							/*
							 * Since the bgworker cannot catch errors due to
							 * signal dependencies, we must silently abort the
							 * current transaction, instead of throwing an
							 * error.
							 */
							if (nbuf != InvalidBuffer)
								_bt_relbuf(rel, nbuf);
