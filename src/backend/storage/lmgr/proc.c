...
#ifdef REPLICATION
CommitOrderId ProcGetLowestCoid(LOCK *lock);
#endif
...
#ifdef REPLICATION
CommitOrderId
ProcGetLowestCoid(LOCK *lock)
{
	PROC_QUEUE	   *waitQueue = &(lock->waitProcs);
	int				queue_size = waitQueue->size;
	PGPROC		   *proc;
	CommitOrderId	proc_coid,
		            coid = InvalidCommitOrderId;
	...
	while (queue_size-- > 0)
	{
		proc_coid = get_local_coid_by_local_xid(proc->xid);
		if (CommitOrderIdIsValid(proc_coid))
		{
			if (CommitOrderIdIsValid(coid))
			{
				Assert(CommitOrderIdsInSameRange(proc_coid, coid));
				if (CmpCommitOrderIds(proc_coid, coid) < 0)
				{
#ifdef COORDINATOR_DEBUG
					elog(DEBUG5, "ProcGetLowestCoid: backend pid %d, xid %d",
						 proc->pid, proc->xid);
					elog(DEBUG5, "ProcGetLowestCoid: has new lowest coid: %d",
						 proc_coid);
#endif
					coid = proc_coid;
				}
...
ProcLockWakeup(LockMethod lockMethodTable, LOCK *lock)
{
...
#ifdef REPLICATION
	lowest_coid = ProcGetLowestCoid(lock);
#endif
	...
	while (queue_size-- > 0)
	{
		 ...
		 *
		 * FIXME: for eager mode Postgres-R, this still works. However, for a
		 *         fully synchronous mode it might be beneficial to control
		 *         which process to wakeup. We need to take care not to miss
		 *         any one process, though.
		 */
		if ((lockMethodTable->conflictTab[lockmode] & aheadRequests) == 0 &&
			LockCheckConflicts(lockMethodTable,
							   lockmode,
