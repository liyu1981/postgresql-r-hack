...
/*
 * BackendXidGetBackendId -- get a backend's id given its XID
 *
 * Returns InvalidBackendId if not found.  Note that it is up to the caller
 * to be sure that the question remains meaningful for long enough for the
 * answer to be used ...
 */
BackendId
BackendXidGetBackendId(TransactionId xid)
{
	BackendId   result = InvalidBackendId;
	ProcArrayStruct *arrayP = procArray;
	int			index;
...
/*
 * CancelConcurrentTransaction - used in replication conflict resolution
 *
 * Returns pid of the process signaled, or 0 if not found.
 */
pid_t
CancelConcurrentTransaction(TransactionId xid)
{
	int index;
	pid_t pid = 0;
	ProcArrayStruct *arrayP = procArray;
	...
	for (index = 0; index < arrayP->numProcs; index++)
	{
		...
		{
			/*
			 * If the transaction has already been aborted, we don't need to
			 * send it another kill signal. However, we still need to return
			 * the pid of the process that is to is going to be aborted.
			 */
			if (proc->abortFlag)
 ...
 * ForeachDBBackend
 *
 * Executes a callback function for each backend which is attached to
 * the given database.
 */
void
ForeachDBBackend(Oid databaseid, perBackendFunc *cb, void *data)
{
	ProcArrayStruct *arrayP = procArray;
	int			index;

	BackendId	to_notify[MaxBackends];
	int			count_pids = 0;

	/*
	 * Quickly fetch all pids when holding the lock, then release
	 * the lock before calling the callback function once for every
	 * pid.
	 */
	LWLockAcquire(ProcArrayLock, LW_SHARED);
	for (index = 0; index < arrayP->numProcs; index++)
	{
		PGPROC	   *proc = arrayP->procs[index];
		if (proc->databaseId == databaseid && proc->pid != 0)
		{
			Assert(count_pids < MaxBackends);
			to_notify[count_pids] = proc->backendId;
			count_pids++;
		}
	}
	LWLockRelease(ProcArrayLock);

	for (index = 0; index < count_pids; index++)
		cb(0, to_notify[index], data);
}
#endif
