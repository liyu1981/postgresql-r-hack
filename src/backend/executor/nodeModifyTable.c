...
#ifdef REPLICATION
		/* exclude recovery processes */
		(!IsBackgroundWorkerProcess()) &&
#endif
...
#else
	/*
	 * FIXME: it should not be necessary to exclude background workers here,
	 *         but ATM we get an error on OIDs (?!?)
	 */
	if (!IsBackgroundWorkerProcess() &&
		(resultRelationDesc->rd_att->constr))
#endif
...
#ifdef REPLICATION
	/* exclude recovery processes */
	if (IsReplicatedBackend())
		cset_collect_tuple(estate, CMD_INSERT, &(tuple->t_self), tuple);
#endif
...
#ifdef REPLICATION
	/*
	 * Add the tuple info to the changeset, except if this is a recovery
	 * process.
	 */
	if (IsReplicatedBackend())
		cset_collect_tuple(estate, CMD_DELETE, tupleid, NULL);
#endif
		   ...
		   EState *estate)
{
	 ...
	 */
	if (IsReplicatedBackend())
		cset_collect_tuple(estate, CMD_UPDATE, tupleid, tuple);
#endif	
...
#ifdef REPLICATION
			if (IsReplicatedBackend())
				cset_close_statement(estate->es_result_relation_info);
#endif
...
#ifdef REPLICATION
				if (IsReplicatedBackend())
					cset_open_statement(estate->es_result_relation_info,
										node->operation);
#endif
				continue;
			}
...
#ifdef REPLICATION
	if (IsReplicatedBackend())
		cset_open_statement(estate->es_result_relations, node->operation);
#endif
...
/*
 *     ExecProcessTuple (helper backend)
 *
 * Applies changes for a single tuple at the helper backend.
 */
void
ExecProcessTuple(IStreamReader reader, EState *estate, CsetStmtInfo *csi)
{
	int 				i,
						attnum,
						numKeys;
	...
	TransactionId		xmin, xmax, origin_xid;
	CommitOrderId		eff_coid,
		                req_coid,
		                my_coid = InvalidCommitOrderId;
	 ...
	 */
	if (IsBootstrapProcessingMode())
		elog(ERROR, "cannot apply change sets during bootstrap");

	Assert(IsBackgroundWorkerProcess());

#ifdef DEBUG_CSET_APPL
	elog(DEBUG3, "bg worker [%d/%d]: processing %s for relation %s\n",
		 MyProcPid, MyBackendId, decode_command_type(csi->type),
		 get_rel_name(csi->localRelOid));
#endif

	oldcontext = MemoryContextSwitchTo(csi->memCtx);

	estate->es_result_relation_info = csi->rr_info;
	Assert(estate->es_crosscheck_snapshot == InvalidSnapshot);

	attrs = csi->tdesc->attrs;

	slot.tts_mcxt = CurrentMemoryContext;
	slot.tts_tupleDescriptor = csi->tdesc;
	...
	{
		/*
		 * For UPDATEs and DELETEs we use the primary key index to find the
		 * target tuple to be updated or deleted.
		 *
		 * 	1. extract the primary key value from the serialized data from
		 *     the change set.
		 *	2. scan the primary key index for existing tuples with these
		 *     primary key values.
		 */
		if (csi->type == CSCMD_UPDATE || csi->type == CSCMD_DELETE)
		{
			...
			skeys = palloc0(idx_rel->rd_index->indnatts * sizeof(ScanKeyData));
			for (i = 0; i < numKeys; i++)
			{
				attnum = idx_info->ii_KeyAttrNumbers[i];

				get_sort_group_operators(attrs[attnum - 1]->atttypid,
										 false, true, false,
										 NULL, &eq_func, NULL);
...
continue_scan:
			...
			if (oldtuple && HeapTupleIsValid(oldtuple))
			{
				...
				buffer = ReadBuffer(csi->base_rel, ItemPointerGetBlockNumber(
										&oldtuple->t_self));
				LockBuffer(buffer, BUFFER_LOCK_EXCLUSIVE);

				xmin = HeapTupleHeaderGetXmin(oldtuple->t_data);
				xmax = HeapTupleHeaderGetXmax(oldtuple->t_data);

#ifdef DEBUG_CSET_APPL
				elog(DEBUG5, "bg worker [%d/%d]: ExecProcessTuple: found a matching tuple: xmin: %d, xmax: %d",
					 MyProcPid, MyBackendId, xmin, xmax);
#endif

				Assert(estate->es_output_cid == FirstCommandId);
				result = HeapTupleSatisfiesUpdate(oldtuple->t_data,
												  estate->es_output_cid,
												  buffer);
				...
				switch (result)
				{
...
#ifdef DEBUG_CSET_APPL
						elog(DEBUG5, "bg worker [%d/%d]: ExecProcessTuple: invisible, skipping",
					...
					case HeapTupleUpdated:
#ifdef DEBUG_CSET_APPL
						elog(DEBUG5, "bg worker [%d/%d]: ExecProcessTuple: updated, must abort",
							 MyProcPid, MyBackendId);
#endif
...
#ifdef DEBUG_CSET_APPL
						elog(DEBUG5, "bg worker [%d/%d]: ExecProcessTuple: concurrent update...",
							 MyProcPid, MyBackendId);
#endif
						resolve_conflict(xmax, &my_coid);
						if (MyProc->abortFlag)
							goto abort_cset_application;
						else
							goto retry_tuple;
				...
				elog(DEBUG5, "bg worker [%d/%d]: ExecProcessTuple: required coid: %d, effective coid: %d",
					 MyProcPid, MyBackendId, req_coid, eff_coid);
#endif

				/*
				 * Some sanity checking of the given commit order ids against
				 * the current process's commit order id.
				 */
				Assert(eff_coid != my_coid);
				Assert(req_coid != my_coid);
			...
			/*
			 * If no matching tuple could be found, this remote transaction
			 * must be aborted with a serialization failure. Looks like
			 * the tuple has already been deleted by a preceeding
			 * transaction (or it never existed, in that case the replicas
			 * are out of sync. Currently we don't try to detect that case).
			 */
			if (oldtuple == NULL)
			{
		...
		{
			if (csi->type == CSCMD_UPDATE)
				Assert(oldtuple);
			else
				Assert(oldtuple == NULL);	// INSERTs

			deserialize_tuple_changes(reader, csi->tdesc, oldtuple,
									  values, nulls);

			newtuple = heap_form_tuple(csi->tdesc, values, nulls);
			 ...
			 */
			newtuple = ExecMaterializeSlot(&slot);
		...
		switch(csi->type)
		{
			case CSCMD_INSERT:
#ifdef DEBUG_CSET_APPL
				elog(DEBUG3, "bg worker [%d/%d]: ExecProcessTuple: calling heap_insert...",
					 MyProcPid, MyBackendId);
#endif

				/*
				 * If we want to support OIDs some day, we'd better transmit
				 * them as part of the tuple. Then make sure the newtuple's
				 * OID is set to that value.
				 */
				Assert(!csi->base_rel->rd_rel->relhasoids);

				/*
				 * Insert the tuple, same as ExecInsert
				 */
				newId = heap_insert(csi->base_rel, newtuple,
									estate->es_output_cid, 0, NULL);
			...
			case CSCMD_DELETE:
#ifdef DEBUG_CSET_APPL
				elog(DEBUG3, "bg worker [%d/%d]: ExecProcessTuple: calling heap_delete...",
					 MyProcPid, MyBackendId);
#endif
				 ...
				 * delete the tuple
				 *
				 * Note that we never wait for locks, but instead need to
				 * react on conflicts.
				 */
				Assert(result == HeapTupleMayBeUpdated);
				heap_delete_internal(csi->base_rel, oldtuple, buffer,
									 estate->es_output_cid);
				break;
			
			case CSCMD_UPDATE:
#ifdef DEBUG_CSET_APPL
				elog(DEBUG3, "bg worker [%d/%d]: calling heap_update...",
					 MyProcPid, MyBackendId);
#endif

				Assert(PointerIsValid(oldtuple));
			...
			default:
				elog(WARNING, "bg worker [%d/%d]: ExecProcessTuple: unknown command",
					 MyProcPid, MyBackendId);
				break;
		}
		...
		if (csi->type == CSCMD_INSERT)
			Assert(!HeapTupleIsHeapOnly(newtuple));
